...
## 1. 기본 원칙: Test-First (TDD)

> "테스트 없이 작성된 복잡한 로직은 부채일 뿐이다."

- **TDD 우선**: 복잡한 유틸리티, 비즈니스 로직, 상태 변환 로직은 구현 전 테스트를 먼저 작성한다.
- **Red-Green-Refactor**: 실패하는 테스트 작성(Red) → 최소한의 구현으로 통과(Green) → 코드 정체성 개선(Refactor) 순서를 준수한다.
- **테스트 없는 코드 삭제**: 핵심 로직 변경 시 테스트가 동반되지 않으면 해당 코드는 반려되거나 삭제될 수 있다.
